...
import { SONG_STYLE_GROUPS } from '../data/songStyles'

/** Number of style groups to show on the homepage; rest on /song-styles */
const GROUPS_ON_HOME = 4
/** Styles per group on home (keep section compact) */
const STYLES_PER_GROUP_ON_HOME = 4

function StyleChip({ name, onCopy }) {
...
export default function SongStyles() {
  ...
  return (
    <section id="song-styles" ref={ref} className="relative py-24 md:py-32 overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
          <p className="text-gold/80 font-cinzel text-sm tracking-[0.3em] uppercase mb-4">Song Styles</p>
          <h2 className="font-cinzel font-bold text-3xl sm:text-4xl text-white">Explore 220+ devotional styles</h2>
          <p className="mt-4 text-slate-400 max-w-xl mx-auto">
            Click any style to copy. Use these in your prompts to shape the mood and sound of your AI-generated songs.
          </p>
        </motion.div>
        <div className="space-y-12">
          {SONG_STYLE_GROUPS.slice(0, GROUPS_ON_HOME).map((group, i) => {
            const preview = group.styles.slice(0, STYLES_PER_GROUP_ON_HOME)
            return (
              <motion.div
                key={group.id}
                initial={{ opacity: 0, y: 24 }}
                animate={inView ? { opacity: 1, y: 0 } : {}}
                transition={{ type: 'tween', ease: [0.25, 0.46, 0.45, 0.94], duration: 0.5, delay: Math.min(i * 0.08, 0.35) }}
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {preview.map((name) => (
                    <StyleChip key={name} name={name} onCopy={copyToClipboard} />
                  ))}
                </div>
              </motion.div>
            )
